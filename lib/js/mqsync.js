	...
	var mqSync = function(){
		...
		if (activeMQ != currentMQ) {
			if (activeMQ === 'MOBILE') {
				...
				isMobile();
				initFavorMobile();
				if($.cookie('noday7') == 'hidden') {
					$('.minwon-pop-wrap').hide();
				} else {
					$('.minwon-pop-wrap').hide();
				}
				// $('dl.notice').clone().appendTo('.m-notice');
			}
			if (activeMQ === 'PC') {
				currentMQ = activeMQ;
				// Add code you want to sync with this breakpoint
				...
				$('.mask-close').on('click', function() {
					if($('#checkCookie').is(':checked')) {
						$.cookie('noday7', 'hidden', {expires : 1});
						$('.minwon-pop-wrap').hide();
					} else {
						$('.minwon-pop-wrap').hide();
					}
				...
				if($.cookie('noday7') == 'hidden') {
					...
					$('.minwon-pop-wrap').slideDown();
				};
				// $('.m-notice dl.notice ').remove();
			}
		}
	}; // End mqSync
